...
void textEditor::editorPad(int starty,int height, int width){
  ...
  while(breakout==0){
	...
	for(int j=0; j<width;j++){
      ...
      if(position != (int)wholeString.length() && wholeString.at(position) != '\n'){
	x++;
      }
      wmove(win,y,x);
      prefresh(win,start,0,starty+1,1,height,width);
	...
	for(int j=0; j<width;j++){
      ...
      wholeString.insert(position,temp);
      wclear(win);
      wprintw(win,wholeString.c_str());
      x++;
      wmove(win,y,x);
